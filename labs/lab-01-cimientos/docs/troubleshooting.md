...
| 8 | El nodo aparece `NotReady` recién creado el clúster. | La red interna (CNI) aún está arrancando. | Espera 30–60 s y vuelve a consultar `kubectl get nodes`. |
| 9 | `kubectl logs deployment/...` dice `Found 2 pods` y muestra logs viejos. | Hay un rollout en curso (el pod viejo muriendo y el nuevo arrancando); el comando eligió el viejo. | Espera a que quede un solo pod (`kubectl get pods -n meridiano-sistema`) y repite el comando. |
| 10 | La creación del clúster falla con un error de imagen de nodo o de containerd, pese a que Docker está bien. | Tu `kind` es anterior a v0.32.0 y no conoce el digest pineado de `kindest/node:v1.34.8`. | Actualiza kind a v0.32.0 o superior: `brew upgrade kind` (macOS) o reinstala el binario en WSL2. Verifica con `kind version` y vuelve a ejecutar `bin/00-verificar-entorno.sh`. |
